...
/*   Updated: 2024/11/30 14:55:59 by llemmel          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	update_screen_movement(t_vars *vars)
{
	...
	if (offset_x < 0)
		render(vars->setting, &vars->img, \
			(double [4]){WIN_WIDTH + offset_x, 0, WIN_WIDTH, WIN_HEIGHT});
	else if (offset_x > 0)
		render(vars->setting, &vars->img, \
			(double [4]){0, 0, offset_x, WIN_HEIGHT});
	if (offset_y < 0)
		render(vars->setting, &vars->img, \
			(double [4]){0, WIN_HEIGHT + offset_y, WIN_WIDTH, WIN_HEIGHT});
	else if (offset_y > 0)
		render(vars->setting, &vars->img, \
			(double [4]){0, 0, WIN_WIDTH, offset_y});
...
void	update_screen_zoom(t_vars *vars)
{
	...
	render(vars->setting, &vars->img, \
		(double [4]){0, 0, WIN_WIDTH, WIN_HEIGHT});
	mlx_clear_window(vars->mlx, vars->win);
	mlx_put_image_to_window(vars->mlx, vars->win, vars->img.img, 0, 0);
}
